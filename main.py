...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if epoch == 0 or epoch == 30 or epoch == 60 or epoch == 90:
            save_checkpoint({
                'epoch': epoch,
                'arch': args.arch,
            ...
            args.alpha = 0.9 if epoch < 60 else 0.5 if epoch < 90 else 0.1 if epoch < 100 else 0.0
        if args.distributed:
...
def train_kd(train_loader, model, teacher, criterion, optimizer, epoch):
    ...
    while input is not None:
        ...
        if args.SKD:
            ...
            loss_kd = nn.KLDivLoss(reduction="batchmean")(F.log_softmax(output / args.T, dim=1), p_t
